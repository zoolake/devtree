// user
export const LOGIN_USER = 'login_user';
export const REGISTER_USER = 'register_user';
export const AUTH_USER = 'auth_user';
export const LOGOUT_USER = 'logout_user';
export const IDCHECK_USER = 'idcheck_user';
export const DETAIL_USER = 'detail_user';
export const GET_CHATS = 'get_chat';
export const AFTER_POST_MESSAGE = 'after_post_message';
export const PASSWORD_UPDATE = 'passwordUpdate';
export const DELETE_USER = 'delete_user';
export const UPDATE_USER = 'update_user';
export const GET_MENTOR_TECH_GET = 'get_mentor_tech_get';
export const DETAIL_MENTOR = 'detail_mentor';
export const GET_REVIEWS = 'get_review';
export const GET_TEAM = 'get_team';
export const SUBMIT_MENTORING = 'submit_mentoring';
export const UPDATE_MENTOR_PROFILE = 'update_mentor_profile';
export const SET_MENTOR = 'set_Mentor';
export const GET_TIMELIST = 'get_timelist';
export const GET_RESERVEDLIST = 'get_reservedlist';
export const GET_MENTORINGLIST = 'get_mentoringlist';
export const GET_RANK = 'get_rank';
export const CHECK_TEAM_MEMBER = 'check_team_member';
export const GET_MY_STUDY_CNT = 'get_my_study_cnt';
export const GET_MY_PROJECT_CNT = 'get_my_project_cnt';
export const AUTH_CODE = 'auth_code';
export const EMAIL_AUTH = 'email_auth';
export const GET_ALARMDATA = 'get_alarmdata';
export const GET_STUDY = 'get_study';
export const GET_PROJECT = 'get_project';
export const AUTH = 'auth';
export const MYMENTOR_PROFILE = 'mymentor_profile';
export const MENTEE_MENTORING = 'mentee_mentoring';
export const CHANGE_STATE = 'change_state';
export const REJECT_MENTORING = 'reject_mentoring';
export const ACCEPT_MENTORING = 'accept_mentoring';
export const GET_CHECKEDTIME_LIST = 'get_checkedtime_list';
export const GET_MENTORS = 'get_mentors';
export const GET_SCHEDULE = 'get_schedule';
export const UPDATE_MENTOR = 'update_mentor';

// project
export const GET_PROJECT_LIST = 'get_proejct_list';
export const CREATE_PROJECT = 'create_project';
export const DELETE_PROJECT = 'delete_project';
export const GET_PROJECT_DETAIL = 'get_project_detail';
export const UPDATE_PROJECT = 'update_project';
export const JOIN_PROJECT = 'join_project';
export const GET_PROJECT_RESPONSE = 'get_project_response';
export const ANSWER_PROJECT_RESPONSE = 'answer_project_response';

// study
export const GET_STUDY_LIST = 'get_study_list';
export const CREATE_STUDY = 'create_study';
export const DELETE_STUDY = 'delete_study';
export const GET_STUDY_DETAIL = 'get_study_detail';
export const UPDATE_STUDY = 'update_study';
export const JOIN_STUDY = 'join_study';
export const GET_STUDY_RESPONSE = 'get_study_response';
export const ANSWER_STUDY_RESPONSE = 'answer_study_response';
export const GET_POSITION_LIST = 'get_position_list';

// team
export const GET_TECH_LIST = 'get_tech_list';
export const CHANGE_TEAM_STATE = 'change_team_state';

//
export const GET_ALARMLIST = 'get_alarm_list';
export const GET_MYMENTORPROFILE = 'get_my_mentorfile';
export const GET_TECH = 'get_tech';
